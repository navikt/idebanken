...
fun BoolQueryBuilder.mustNotHaveMetatags(vararg metatags: ValidMetatags) =
    apply {
        metatags.forEach { mustNot(TermQueryBuilder(METATAGS, it.descriptor)) }
    }

fun BoolQueryBuilder.mustHaveOneOfMetatags(vararg metatags: ValidMetatags) =
    apply {
        must(BoolQueryBuilder().shouldHaveMetatags(*metatags))
    }

fun BoolQueryBuilder.shouldHaveMetatags(vararg metatags: ValidMetatags) =
    apply {
        metatags.forEach { should(TermQueryBuilder(METATAGS, it.descriptor)) }
    }

fun BoolQueryBuilder.mustHaveTypes(vararg types: ValidTypes) =
    apply {
        types.forEach { must(TermQueryBuilder(TYPE, it.descriptor)) }
    }

fun BoolQueryBuilder.mustNotHaveTypes(vararg types: ValidTypes) =
    apply {
        types.forEach { mustNot(TermQueryBuilder(TYPE, it.descriptor)) }
    }

fun BoolQueryBuilder.mustHaveOneOfTypes(vararg types: ValidTypes) =
    apply {
...
fun BoolQueryBuilder.mustNotHaveField(fieldName: String) =
    apply {
        mustNot(ExistsQueryBuilder(fieldName))
    }

fun BoolQueryBuilder.mustHaveAudience(
...
fun preferredLanguageFilterQuery(preferredLanguage: String) =
...
fun Collection<BoolQueryBuilder>.joinToSingleQuery(operator: BoolQueryBuilder.(QueryBuilder) -> BoolQueryBuilder): BoolQueryBuilder {
