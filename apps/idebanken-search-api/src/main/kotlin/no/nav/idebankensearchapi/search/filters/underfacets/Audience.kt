...
private fun informasjonFilter(audience: ValidAudiences) = baseFilter(audience).mustHaveMetatags(ValidMetatags.INFORMASJON)

private fun kontorFilter(audience: ValidAudiences) = baseFilter(audience).mustHaveOneOfTypes(ValidTypes.KONTOR, ValidTypes.KONTOR_LEGACY)

private fun soknadOgSkjemaFilter(audience: ValidAudiences) =
    baseFilter(audience).mustHaveOneOfTypes(
        ValidTypes.SKJEMA,
        ValidTypes.SKJEMAOVERSIKT,
    )

private fun aktueltFilter(audience: ValidAudiences) =
    baseFilter(audience, isStrict = true).mustHaveMetatags(
        ValidMetatags.NYHET,
    )
