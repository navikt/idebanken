...
function wrapReplacer(
	oldReplacer: OldReplacer | undefined,
	meta: MetaData,
...
function MacroAdapter(props: MacroComponentParams<ExtraRichTextProps>) {
	...
	return (
		<BaseMacro data={data} meta={meta} renderInEditMode={renderInEditMode}>
			{children}
		</BaseMacro>
	)
}
...
function ImageAdapter(props: ImageComponentParams<ExtraRichTextProps>) {
	const srcSet = props.srcSet?.length
		? UrlProcessor.processSrcSet(props.srcSet, props.meta)
		: undefined
	return (
		<img
			src={getUrl(props.src, props.meta)}
			style={props.style}
			alt={props.alt}
			sizes={props.sizes}
			srcSet={srcSet}
